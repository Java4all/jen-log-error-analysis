...
function ConfigPanel({ serverConfig, onSaved }) {
  ...
  return (
    <div style={{ maxWidth:800 }}>
        ...
        {testResults[cfg.ai.provider] && (
          <div style={{ marginTop:8, padding:"6px 12px", borderRadius:6, fontSize:12,
            ...
            {testResults[cfg.ai.provider].status==="ok"
              ? `✅ Connected: ${testResults[cfg.ai.provider].response}`
              : `❌ ${testResults[cfg.ai.provider].error}`}
          </div>
        )}
      </div>
          ...
          {cfg.github.repos.map((repo, i) => (
            <div key={i} style={{ background:"#0d1117", borderRadius:6, padding:12, marginBottom:8, border:"1px solid #21262d" }}>
              <div style={S.row}>
                ...
                <Btn small onClick={() => removeRepo(i)} variant="danger">✕</Btn>
              </div>
              <div style={S.row}>
                <div style={{ flex:1 }}>
          ...
          {testResults.gh && (
            <div style={{ padding:"6px 12px", borderRadius:6, fontSize:12, marginTop:4,
              ...
              {testResults.gh.status==="ok"
                ? `✅ ${testResults.gh.total_files} files found, ${testResults.gh.matching_files} matching. Sample: ${testResults.gh.sample?.slice(0,3).join(", ")}`
                : `❌ ${testResults.gh.error}`}
            </div>
          )}
        </div>
      </div>
      ...
      {/* Pipeline Tags */}
      <div style={S.section}>
        <div style={S.h}>🏷 Pipeline Tags</div>
        ...
        {cfg.pipeline.static_tags.map((tag, i) => (
          <div key={i} style={{ ...S.row, marginBottom:6 }}>
            ...
            <Btn small onClick={() => removeTag(i)} variant="danger">✕</Btn>
          </div>
        ))}
      ...
      {/* Analysis */}
      <div style={S.section}>
        <div style={S.h}>⚙️ Analysis Settings</div>
        <div style={S.row}>
          <div style={{ flex:1 }}>
      ...
      <div style={{ display:"flex", gap:10, alignItems:"center" }}>
        <Btn onClick={save} loading={saving}>💾 Save Config</Btn>
        {msg && <span style={{ fontSize:12, color: msg.startsWith("✅")?"#3fb950":"#ff7b72" }}>{msg}</span>}
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  useEffect(() => {
    apiFetch("/health")
      .then(h => { setBackendStatus(true); setServerConfig(null); loadConfig(); })
      .catch(() => setBackendStatus(false));
  }, []);

  const loadConfig = async () => {
    try { const cfg = await apiFetch("/api/config"); setServerConfig(cfg); } catch {}
  };
  ...
  const tabs = [
    ...
    { id:"report", label:"🤖 AI Report", disabled: !parsed },
    { id:"config", label:"⚙ Config",    badge: backendStatus ? "API ✓" : "offline" },
  ];
      ...
      <div style={{ display:"flex", borderBottom:"1px solid #21262d", background:"#161b22", padding:"0 28px" }}>
        {tabs.map(t => (
          <button key={t.id} disabled={t.disabled} onClick={() => !t.disabled && setActiveTab(t.id)} style={{
            ...
            color: activeTab===t.id ? "#58a6ff" : t.disabled ? "#484f58" : "#8b949e",
        ...
        {activeTab === "input" && (
          <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
            ...
            <div style={{ background:"#161b22", border:"1px solid #30363d", borderRadius:8, padding:14 }}>
              <div style={{ fontSize:11, color:"#8b949e", marginBottom:6, textTransform:"uppercase", letterSpacing:1 }}>🏷 Pipeline Tags Override (comma-separated, empty = use config)</div>
            ...
            <div style={{ background:"#161b22", border:"1px solid #30363d", borderRadius:8, padding:14 }}>
              <div style={{ fontSize:11, color:"#8b949e", marginBottom:8, textTransform:"uppercase", letterSpacing:1 }}>📂 Load from URL or File</div>
              ...
              <div style={{ display:"flex", gap:8 }}>
                <Btn onClick={() => fileInputRef.current.click()} variant="secondary">📁 Upload .txt</Btn>
            ...
            <div style={{ background:"#161b22", border:"1px solid #30363d", borderRadius:8, padding:14 }}>
              <div style={{ fontSize:11, color:"#8b949e", marginBottom:8, textTransform:"uppercase", letterSpacing:1 }}>📋 Paste Log</div>
        ...
        {activeTab === "parse" && parsed && (
          <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
            ...
            <div style={{ background:"#161b22", border:"1px solid #21262d", borderRadius:8, padding:16 }}>
              ...
              <div style={{ fontSize:10, color:"#484f58", marginTop:4 }}>🔴 Red bars = above slow threshold percentile</div>
            </div>
        ...
        {activeTab === "tree" && parsed && (
          <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
            <div style={{ display:"flex", gap:8, alignItems:"center" }}>
              ...
              <Btn onClick={() => { setActiveTab("report"); if (!parsed.ai_report) regenerateAI(); }} loading={aiLoading} style={{ marginLeft:"auto" }}>🤖 AI Report</Btn>
            </div>
            <div style={{ background:"#161b22", border:"1px solid #21262d", borderRadius:8, padding:16 }}>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
            <div style={{ display:"flex", gap:8, alignItems:"center" }}>
              <Btn onClick={regenerateAI} loading={aiLoading} disabled={!backendStatus}>
                🤖 {parsed?.ai_report ? "Regenerate" : "Generate AI Report"}
              </Btn>
              {!backendStatus && <span style={{ fontSize:11, color:"#ff7b72" }}>Backend required for AI reports — start the Python API server</span>}
              {parsed?.ai_report && <Btn onClick={() => navigator.clipboard.writeText(parsed.ai_report)} variant="ghost">📋 Copy</Btn>}
            </div>
